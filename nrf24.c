...
void nrf24_TX(void *args) {
	(void)args;
  //TODO wait for adc control notifcations or uart1 notifcations to send commands ( security )
	 uint8_t temp = 0 ;
	for(;;)
	{
		if(TX_mode == 0) //send nunchunk package type
		{
			uint8_t data[sizeof(nunchuck)+1];
			data[0] = 1; // set the type of frame (nunchuck)

			memcpy(&data[1],&nunchuck,sizeof(nunchuck));
		...
		else if (TX_mode == 1) // send rpm packag type
		{
			uint8_t data[sizeof(RPM_VESC)+1];
			data[0] = 2; // set the type of frame (RPM)

			memcpy(&data[1],&RPM_VESC,sizeof(RPM_VESC));
